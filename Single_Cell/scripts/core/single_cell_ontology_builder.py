...
class OntologyBuilder(ABC):
    ...
    @abstractmethod
    def add_data(self, df, coexpression_df_dict=None):
...
class GeneExpressionOntologyBuilder(OntologyBuilder):
    ...
    def create_classes(self):
        ...
        self.graph.add((self.EX.CoexpressedWith, RDF.type, self.EX.Property))
        self.graph.add((self.EX.CoexpressionScore, RDF.type, self.EX.DatatypeProperty))
    ...
    def add_data(self, df, coexpression_df_dict=None):
        """
        Add average gene expression data to the ontology.
        """
